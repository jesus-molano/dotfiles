...
git -C "$fixture_repo" -c user.name=Test -c user.email=test.invalid@example.invalid \
...
# Noctalia keeps its base config under Stow. A local override replaces only the
# generated sibling, and rollback must restore an existing Stow symlink exactly.
generated_noctalia_home="$test_root/generated-noctalia-home"
generated_noctalia_state="$test_root/generated-noctalia-state"
generated_noctalia_config="$generated_noctalia_home/.config"
mkdir -p "$generated_noctalia_home" "$generated_noctalia_state/dotfiles/staged/noctalia" \
	"$fixture_repo/noctalia/.config/noctalia"
printf '%s\n' base-override >"$fixture_repo/noctalia/.config/noctalia/zz-host-overrides.toml"
stow --no-folding --dir "$fixture_repo" --target "$generated_noctalia_home" noctalia
printf '%s\n' generated-override >"$generated_noctalia_state/dotfiles/staged/noctalia/zz-host-overrides.toml"
generated_noctalia_runner="$test_root/generated-noctalia.sh"
cat >"$generated_noctalia_runner" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
export DOTFILES_INSTALL_SOURCE_ONLY=1
source "$FIXTURE_REPO/install.sh"
PLAN_JSON=$TEST_PLAN
PLAN_MODULES=()
BACKUP_DIR="$STATE_DIR/backups/generated-noctalia"
target="$XDG_CONFIG_HOME/noctalia/zz-host-overrides.toml"
before="$(readlink "$target")"
begin_migration
install_staged_file noctalia "$STATE_DIR/staged/noctalia/zz-host-overrides.toml"
[[ -f "$target" && ! -L "$target" && $(<"$target") == generated-override ]]
rollback_migration
[[ -L "$target" && $(readlink "$target") == "$before" && $(<"$target") == base-override ]]
EOF
chmod +x "$generated_noctalia_runner"
HOME="$generated_noctalia_home" XDG_CONFIG_HOME="$generated_noctalia_config" XDG_STATE_HOME="$generated_noctalia_state" \
	FIXTURE_REPO="$fixture_repo" TEST_PLAN="$plan" "$generated_noctalia_runner"
...
# El contrato de solo lectura se valida contra main(), no solo contra funciones
# cargadas con DOTFILES_INSTALL_SOURCE_ONLY. Una instalación nueva sin elecciones
# puede revisar la base segura; el HOME del llamador debe seguir vacío. Sin
# --check/--safe-defaults y sin terminal, la misma ruta debe rechazar el host
# incompleto antes de crear estado local.
e2e_home="$test_root/e2e-home"
e2e_state="$test_root/e2e-state"
e2e_cache="$test_root/e2e-cache"
e2e_data="$test_root/e2e-data"
e2e_bin="$test_root/e2e-bin"
mkdir -p "$e2e_home" "$e2e_state" "$e2e_cache" "$e2e_data" "$e2e_bin"
cat >"$e2e_bin/shelly" <<'EOF'
#!/usr/bin/env bash
...
grep -Fq 'Falta host.toml en una ejecución no interactiva' "$test_root/e2e-missing-host.out"
[[ -z "$(find "$e2e_home" -mindepth 1 -print -quit)" ]]
